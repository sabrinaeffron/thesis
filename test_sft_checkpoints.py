...
for i in nums:
    model_dir = "/scratch/gpfs/GRIFFITHS/se1854/outputs/"+model_dir_name+"_swap"+str(seed)+"/checkpoint-"+str(i)
    print(model_dir)
    ...
    df["bce"] = bce
    df["sq_err"] = sq_err
    df.to_csv(output_dir+"test_results_"+model_name_short+"_swap"+str(seed)+"_"+str(i)+".csv", index=False)
    print("Saved predictions")

    bad_ex = str(df["pred_A"].isnull().sum()), "/", str(len(df))
    bad.append(bad_ex)
    print("Number of bad rows: ", bad_ex)

    avg_bce = np.nanmean(bce)
    bces.append(avg_bce)
    print("Average Binary Cross Entropy Loss for checkpoint "+str(i)+f": {avg_bce:.4f}")

    mse = np.nanmean(sq_err)
    mses.append(mse)
    print("Average Mean Sqaured Error for checkpoint "+str(i)+f": {mse:.4f}")

print("Average Binary Cross Entropy Loss for each checkpoint: ", bces)
print("Average Mean Squared Error for each checkpoint: ", mses)
print("Number of bad rows for each checkpoint: ", bad)